...
fun filterMovie(movieId: String, movieModel: ViewMovieModel): Movie {
    return movieModel.movies.filter { it.id == movieId}[0]
}
@Composable
fun DetailScreen(
    ...
    movieId?.let {
        val movie = filterMovie(movieId = movieId, movieModel = viewMovieModel)
